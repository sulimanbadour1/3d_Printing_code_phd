...
modelWeight = 'Models\\custom-yolov4-tiny-detector_best.weights'     # YOLO weight file location
...
def findObjects(img):
        ...
        blob = cv2.dnn.blobFromImage(img, 1 / 255, (320, 320), [0, 0, 0], 1,
                                     crop=False)  # Converts video feed into blobs
        model.setInput(blob)
        ...
        for i in indices:
                ...
                cv2.putText(img, f'{classNames[classIds[i]].upper()}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (0, 255, 0), 3)

                recordData(classNames[classIds[
                        i]].upper())  # Calls the RecordData function purposed to record detected fault and the time it happened within a text file

        # FPS Calculation
        end_time = time.time()
        elapsed_time = end_time - start_time
        fps = 1 / elapsed_time
        cv2.putText(img, str(round(fps, 2)), (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0),
                    2)  # Displays the FPS to the video feed
